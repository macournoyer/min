SRC = kernel/closure.c kernel/vtable.c kernel/object.c kernel/message.c kernel/string.c kernel/number.c kernel/array.c kernel/lobby.c kernel/grammar.c kernel/scanner.c kernel/min.c
OBJ = ${SRC:.c=.o}
...
size: clean
	@ruby -e 'puts "%0.2fK" % (Dir["kernel/**.{c,rl,h}"].inject(0) {|s,f| s += File.size(f)} / 1024.0)'
